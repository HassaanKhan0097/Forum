...
<body>
    ...
    <main id="tt-pageContent">
        <div class="container">
                    ...
                    <div class="tt-single-topic">
                        <div class="tt-item-header">
                            <div class="tt-item-info info-top">
                                ...
                                <a href="#" class="tt-info-time">
                                    <i class="tt-icon"><svg>
                                        ...
                                        </svg></i><?php echo $post->post_created;?>
                                </a>
                            </div>
                            ...
                            <div class="tt-item-tag">
                                <ul class="tt-list-badge">
                                    <li><a href="#"><span class="tt-color03 tt-badge"><?php echo $post->ch_title;?></span></a></li>
                                </ul>
                            </div>
                        </div>
                        ...
                        <div class="tt-item-description">
                            <?php 
                                ...
                                if($post->post_type == 'Discussion' || $post->post_type == 'Question') { ?>
                            ...
                            <?php }
                                ...
                                $arrayPollOptions = unserialize($post->post_body);

                                foreach ($arrayPollOptions as $opt) { ?>
                            ...
                            <?php 
                            if(preg_match('/(\.mp4|\.wmv|\.webm)$/i', $post->post_body)) { ?>
                            <video width="100%" height="200%" controls>
                                <source src="<?php echo $post->post_body; ?>" type="video/mp4">
                            </video>
                            <?php } else { ?>
                            <iframe width="100%" height="200%" src="<?php echo $post->post_body; ?>" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
                            <?php } ?>
                            ...
                            <?php }
                                ...
                                <img src="<?php echo $post->post_body; ?>" alt="">
                        ...
                        <div class="tt-item-info info-bottom">
                            <?php if ($isPostLiked) { $class_name = 'tt-icon-btn linkToButton tt-icon-btn-active'; } else { $class_name = 'tt-icon-btn linkToButton'; } ?>
                            <button class="<?php echo $class_name; ?>" id="likePostBtn">
                                <i class="tt-icon"><svg>
                                        <use xlink:href="#icon-like"></use>
                                    </svg></i>
                                <span class="tt-text">
                                    <?php $arrayLikes = unserialize($post->post_likes); ?>
                                    <?php echo count($arrayLikes); ?>
                                </span>
                            </button>
                            ...
                            <a href="#divReply" class="tt-icon-btn tt-hover-02 tt-small-indent">
                                <i class="tt-icon"><svg>
                    ...
                    <div class="tt-single-topic">
                        <div class="tt-item-header pt-noborder">
                            <div class="tt-item-info info-top">
                                ...
                                <div class="tt-avatar-title">
                                    <a href="#"><?php echo $pc->user_username; ?></a>
                                </div>
                                <a href="#" class="tt-info-time">
                                    <i class="tt-icon"><svg>
                                            <use xlink:href="#icon-time"></use>
                                        </svg></i><?php echo $pc->pc_created; ?>
                                </a>
                        ...
                        <div class="tt-item-description">
                            <?php echo $pc->pc_comment; ?>
                        </div>
                        <div class="tt-item-info info-bottom">

                        <!-- Check if comment is liked by loggedUser -->
                        <?php 
                            $isCommentLiked = false;
                            $arrayLikesComments = unserialize($pc->pc_likes);
                            if (in_array($this->session->userdata('loggedUser')['user_id'], $arrayLikesComments)) {
                                $isCommentLiked = true;
                            }                
                        ?>
                        <?php if ($isCommentLiked) { $class_name = 'tt-icon-btn linkToButton tt-icon-btn-active'; } else { $class_name = 'tt-icon-btn linkToButton'; } ?>
                            <button class="<?php echo $class_name; ?>" id="likeCommentBtn" name="<?php echo $pc->pc_id; ?>">
                                <i class="tt-icon"><svg>
                                        <use xlink:href="#icon-like"></use>
                                    </svg></i>
                                <span class="tt-text">
                                    <?php echo count($arrayLikesComments); ?>
                                </span>
                            </button>
                            ...
                            <?php 
                                // var_dump($post_pledges);
                                $pledge = null;
                                foreach ($post_pledges as $obj) {
                                    if($obj->pcp_pc_id == $pc->pc_id && $obj->pcp_user_id == $this->session->userdata('loggedUser')['user_id']){
                                        $pledge = $obj;
                                    }
                                }
                                
                                //count pledges
                                $output['money'] = 0; $output['time'] = 0; $output['contact'] = 0;
                                foreach ($post_pledges as $obj) {
                                    if($obj->pcp_pc_id == $pc->pc_id){
                                        $output[$obj->pcp_pledge] = ($output[$obj->pcp_pledge] ?? 0) + 1;
                                    }
                                }
                            ?>

                                <div class="form-group">
                                    <!-- <select class="form-control" name="post_ch_id">
                                        <option selected="" disabled="" value="0"><i class="fa fa-money fa-lg"></i></option>                                                                                                     
                                            <option value="1">politics</option>                             
                                            <option value="2">video</option>                             
                                    </select> -->
                                    <select name="<?php echo $pc->pc_id; ?>" id="selectPledge<?php echo $pc->pc_id; ?>" onChange="onSelectPledgeChange(<?php echo $pc->pc_id; ?>)" style='height: 45px; font-family:Arial, FontAwesome; border:none; outline:none;'>
                                        <option value='-1' >&#xf039; &nbsp; Pledge</option>
                                        <option value='money' <?php echo ($pledge && $pledge->pcp_user_id == $this->session->userdata('loggedUser')['user_id'] && $pledge->pcp_pledge == 'money') ? 'selected' : '' ?> >&#xf0d6; &nbsp; Money (<?php echo $output['money']; ?>)</option>
                                        <option value='time' <?php echo ($pledge && $pledge->pcp_user_id == $this->session->userdata('loggedUser')['user_id'] && $pledge->pcp_pledge == 'time') ? 'selected' : '' ?> >&#xf017; &nbsp; Time (<?php echo $output['time']; ?>)</option>
                                        <option value='contact' <?php echo ($pledge && $pledge->pcp_user_id == $this->session->userdata('loggedUser')['user_id'] && $pledge->pcp_pledge == 'contact') ? 'selected' : '' ?> >&#xf095; &nbsp; Contact (<?php echo $output['contact']; ?>)</option>
                                    </select>
            ...
            <div class="tt-wrapper-inner" id="divReply">
                ...
                <form method="post" action="<?php echo base_url(); ?>posts/create_comment_submit">
                    <div class="form-group">
                    <textarea name="pc_comment" id="pc_comment" class="form-control" rows="5" placeholder="Lets get started"><?php echo set_value('pc_comment'); ?></textarea>
                    <span class="helper-text"><?php echo form_error('pc_comment'); ?></span>
                    </div>
                    <input type="hidden" name="pc_post_id" id="pc_post_id">
                    <div class="pt-row">
                        <div class="col-auto">
                        ...
                        <div class="col-auto">
                            <button type="submit" id="btnReply" class="btn btn-secondary btn-width-lg">Reply</button>
                        </div>
                    </div>
                </form>
                </div>
...
return new Promise((resolve, reject) => {
    $.ajax({
        url: base_url+"posts/addlike/"+post_id,
        type: "GET",
        dataType: "json",
...
function addlikeToComment(pc_id) {
var base_url = "<?php echo base_url(); ?>";
return new Promise((resolve, reject) => {
    $.ajax({
        url: base_url+"posts/addLikeToComment/"+pc_id,
        type: "GET",
        dataType: "json",
        success: function (data) {
            console.log("Success");
...
function addPledgeToComment(pc_id, pledge, pc_post_id) {
var base_url = "<?php echo base_url(); ?>";
return new Promise((resolve, reject) => {
    $.ajax({
        url: base_url+"posts/addPledgeToComment/"+pc_id+"/"+pledge+"/"+pc_post_id,
        type: "GET",
        dataType: "json",
        success: function (data) {
            console.log("Success");
...
$(document).ready(function() {
    ...
    $("#likeCommentBtn").click(function() {
        ...
        var pc_id = this.name; 
        addlikeToComment(pc_id).then(response => {
            console.log("response -->",response)
            window.location.reload();
        })
        .catch(error => {
    ...
    // $('#selectPledge').on('change', function() {
    
    //     var pc_id = this.name;
    //     var pledge = this.value; 
    //     var pc_post_id = window.location.pathname.split("/").pop();
    //     addPledgeToComment(pc_id, pledge, pc_post_id).then(response => {
    //         debugger
    //         console.log("response -->",response)
    //         window.location.reload();
    //     })
    ...
    setPostIdToHiddenFields();
    
    //disable reply button if textarea is empty
    $('#btnReply').attr('disabled', true);
    $('#pc_comment').on('keyup', function () {
        var textarea_value = $("#pc_comment").val();
        if (textarea_value != '') {
...
function setPostIdToHiddenFields(){
    var post_id = window.location.pathname.split("/").pop();
    $("#pc_post_id").val(post_id);
}

function onSelectPledgeChange(pc_id){

        var pledge = $("#selectPledge"+pc_id).val();
        var pc_post_id = window.location.pathname.split("/").pop();
        addPledgeToComment(pc_id, pledge, pc_post_id).then(response => {
            debugger
            console.log("response -->",response)
